...
PYBIND11_MODULE(my_cpp, m)
{
  ...
  py::class_<Bundler, std::shared_ptr<Bundler>>(m, "Bundler")
    ...
    .def("saveNewframeResult", static_cast<void (Bundler::*)(void)>(&Bundler::saveNewframeResult), py::call_guard<py::gil_scoped_release>())
    .def("saveNewframeResult", static_cast<void (Bundler::*)(const std::string &)>(&Bundler::saveNewframeResult), py::call_guard<py::gil_scoped_release>())
    ...
    .def("loadKeyframes", py::overload_cast<const py::array_t<int> &, size_t , const py::array_t<float> &, const Eigen::Matrix3f &, std::string , std::shared_ptr<YAML::Node> >(&Bundler::loadKeyframes), py::call_guard<py::gil_scoped_release>())
    .def("loadTemplates", py::overload_cast<const std::vector<std::string> &, const std::vector<std::string> &, const std::vector<std::string> &, const py::array_t<float> &, const Eigen::Matrix3f &, std::shared_ptr<YAML::Node>>(&Bundler::loadTemplates), py::call_guard<py::gil_scoped_release>())
    ...
    .def_readwrite("_template_frames", &Bundler::_template_frames)
