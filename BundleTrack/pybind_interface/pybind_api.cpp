...
PYBIND11_MODULE(my_cpp, m)
{
  ...
  py::class_<Bundler, std::shared_ptr<Bundler>>(m, "Bundler")
    ...
    .def("loadKeyframes", py::overload_cast<const std::vector<std::string> &, const std::vector<std::string> &, const std::vector<std::string> &, const py::array_t<float> &, const Eigen::Matrix3f &, std::shared_ptr<YAML::Node>>(&Bundler::loadKeyframes), py::call_guard<py::gil_scoped_release>())
